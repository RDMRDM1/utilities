...
local function createMenu()
  ...
  MachoMenuSetAccent(MenuWindow, 255, 0, 0) -- Red

  SectionOneGroup = MachoMenuGroup(MenuWindow, "SectionOne", SectionOneStart.x, SectionOneStart.y, SectionOneEnd.x, SectionOneEnd.y)
  SectionTwoGroup = MachoMenuGroup(MenuWindow, "SectionTwo", SectionTwoStart.x, SectionTwoStart.y, SectionTwoEnd.x, SectionTwoEnd.y)
  SectionThreeGroup = MachoMenuGroup(MenuWindow, "SectionThree", SectionThreeStart.x, SectionThreeStart.y, SectionThreeEnd.x, SectionThreeEnd.y)

  for i, name in ipairs(tabs) do
    MachoMenuButton(SectionOneGroup, name, function()
      selectedTab = i
      buildContent()
    end)
  end
...
local function buildContent()
  MachoMenuDestroyGroup(SectionTwoGroup)
  MachoMenuDestroyGroup(SectionThreeGroup)

  SectionTwoGroup = MachoMenuGroup(MenuWindow, "SectionTwo", SectionTwoStart.x, SectionTwoStart.y, SectionTwoEnd.x, SectionTwoEnd.y)
  SectionThreeGroup = MachoMenuGroup(MenuWindow, "SectionThree", SectionThreeStart.x, SectionThreeStart.y, SectionThreeEnd.x, SectionThreeEnd.y)

  if selectedTab == 1 then -- Self tab
    ...
    MachoMenuKeybind(SectionTwoGroup, "Toggle NoClip - Key: U", 0x55, function(key, toggle)  -- 0x55 is VK_U
      noclipActive = toggle
...
RegisterKeyMapping("toggleMenu", "Toggle Macho Menu", "keyboard", "CAPSLOCK")
